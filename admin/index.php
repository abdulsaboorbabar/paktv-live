...
try {
    ...
    $countryQuery = $db->query("SELECT country, COUNT(*) as count FROM channels GROUP BY country ORDER BY count DESC");
    $countries = $countryQuery->fetchAll();

    $categoryQuery = $db->query("SELECT category, COUNT(*) as count FROM channels GROUP BY category ORDER BY count DESC");
...
<body>
    <div class="container">
        <header>
            <div>
                <h1>PAKTV Live</h1>
                <small style="color: var(--text-muted)">Administration Panel</small>
            </div>
        ...
        <div class="grid">
            <div class="card">
                ...
                <p>Fetch latest public live IPTV playlists and channel catalogs from global repositories matching local filters.</p>
            ...
            <div class="card">
                ...
                <div style="display: flex; justify-content: space-around;">
                    <div>
                        <div class="stat-val"><?php echo $totalChannels; ?></div>
                        <small>Total Channels</small>
                    </div>
                    <div>
                        <div class="stat-val"><?php echo $hdCount; ?></div>
                        <small>HD Streams</small>
                    </div>
                </div>
                <form method="POST" onsubmit="return confirm('Are you sure you want to delete all channel listings?');" style="margin-top: 1.5rem; text-align: right;">
